"use client";

import { useEffect, useMemo, useState } from "react";

import AudioStack from "@/components/AudioStack";
import { getFlashcards } from "@/lib/flashcards";
import { SOUND_CATEGORIES, FlashcardMode, SoundCategory } from "@/lib/types";

const CATEGORY_LABELS: Record<SoundCategory, string> = {
  heart: "심음",
  lung: "폐음",
};

const FLASHCARD_MODES: Array<{ value: FlashcardMode; label: string; helper: string }> = [
  {
    value: "name-first",
    label: "명칭 → 소리",
    helper: "명칭을 보고 소리를 들으며 특징을 떠올립니다.",
  },
  {
    value: "audio-first",
    label: "소리 → 명칭",
    helper: "소리만 듣고 명칭과 설명을 맞춰봅니다.",
  },
];

export default function FlashcardSection() {
  const [category, setCategory] = useState<SoundCategory>("heart");
  const [mode, setMode] = useState<FlashcardMode>("name-first");
  const [index, setIndex] = useState(0);
  const [flipped, setFlipped] = useState(false);

  const cards = useMemo(() => getFlashcards(category, mode), [category, mode]);
  const current = cards[index];

  useEffect(() => {
    setIndex(0);
    setFlipped(false);
  }, [category, mode]);

  const move = (direction: "prev" | "next") => {
    setIndex((prev) => {
      if (direction === "prev") {
        return prev === 0 ? cards.length - 1 : prev - 1;
      }
      return prev === cards.length - 1 ? 0 : prev + 1;
    });
    setFlipped(false);
  };

  return (
    <section className="rounded-3xl bg-white/80 p-6 shadow-sm backdrop-blur">
      <header className="flex flex-col gap-2">
        <h2 className="text-2xl font-semibold text-slate-900">플래시카드</h2>
        <p className="text-sm text-slate-600">
          소리를 외우는 두 가지 학습 모드로 반복 학습하세요. 설명은 카드를 뒤집어 확인할 수 있습니다.
        </p>
      </header>

      <div className="mt-6 flex flex-wrap gap-2">
        {SOUND_CATEGORIES.map((item) => (
          <button
            key={item}
            type="button"
            onClick={() => setCategory(item)}
            className={`rounded-full border px-4 py-2 text-sm font-medium transition-colors ${
              category === item
                ? "border-slate-900 bg-slate-900 text-white"
                : "border-slate-300 bg-white text-slate-700 hover:border-slate-400"
            }`}
          >
            {CATEGORY_LABELS[item]}
          </button>
        ))}
      </div>

      <div className="mt-4 flex flex-wrap gap-2">
        {FLASHCARD_MODES.map((item) => (
          <button
            key={item.value}
            type="button"
            onClick={() => setMode(item.value)}
            className={`rounded-full border px-4 py-2 text-xs sm:text-sm transition-colors ${
              mode === item.value
                ? "border-blue-600 bg-blue-600 text-white"
                : "border-slate-300 bg-white text-slate-700 hover:border-slate-400"
            }`}
          >
            <div className="font-medium">{item.label}</div>
            <div className="hidden text-[11px] text-slate-500 sm:block">{item.helper}</div>
          </button>
        ))}
      </div>

      {current ? (
        <div className="mt-6 flex flex-col gap-4">
          <div className="min-h-[220px] rounded-2xl border border-slate-200 bg-slate-50 p-6 shadow-inner transition-transform">
            {!flipped ? (
              <div className="flex h-full flex-col gap-4">
                {mode === "name-first" ? (
                  <>
                    <h3 className="text-xl font-semibold text-slate-900">{current.name}</h3>
                    <p className="text-sm text-slate-600">
                      소리를 듣고 어떤 장면인지 떠올려보세요. 설명은 카드를 뒤집어 확인합니다.
                    </p>
                  </>
                ) : (
                  <>
                    <h3 className="text-xl font-semibold text-slate-900">소리를 듣고 명칭을 추측해보세요</h3>
                    <p className="text-sm text-slate-600">뒤집기 전까지 명칭과 설명은 숨겨져 있습니다.</p>
                  </>
                )}
                <AudioStack audio={current.audio} />
              </div>
            ) : (
              <div className="flex h-full flex-col gap-4">
                <h3 className="text-xl font-semibold text-slate-900">
                  {mode === "audio-first" ? current.name : "설명"}
                </h3>
                <p className="whitespace-pre-wrap text-sm leading-relaxed text-slate-700">
                  {current.description.trim()}
                </p>
                {mode === "name-first" && (
                  <div>
                    <h4 className="text-sm font-semibold text-slate-800">다시 들으면서 정리하기</h4>
                    <AudioStack audio={current.audio} hideLabels />
                  </div>
                )}
              </div>
            )}
          </div>

          <div className="flex flex-col items-center gap-3 sm:flex-row sm:justify-between">
            <div className="flex items-center gap-2">
              <button
                type="button"
                onClick={() => move("prev")}
                className="rounded-full border border-slate-300 px-4 py-2 text-sm font-medium text-slate-700 hover:border-slate-400"
              >
                이전
              </button>
              <button
                type="button"
                onClick={() => setFlipped((prev) => !prev)}
                className="rounded-full border border-blue-600 bg-blue-600 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-blue-500"
              >
                카드 뒤집기
              </button>
              <button
                type="button"
                onClick={() => move("next")}
                className="rounded-full border border-slate-300 px-4 py-2 text-sm font-medium text-slate-700 hover:border-slate-400"
              >
                다음
              </button>
            </div>
            <div className="text-sm text-slate-600">
              {index + 1} / {cards.length}
            </div>
          </div>
        </div>
      ) : (
        <p className="mt-6 text-sm text-slate-600">선택한 조건에 해당하는 카드가 없습니다.</p>
      )}
    </section>
  );
}
